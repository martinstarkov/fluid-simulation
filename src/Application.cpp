...
public:
    ...
    void Update() {
        engine::Timer t1;
        t1.Start();
        SYCLDiffuseVelocities();
        engine::PrintLine("diffuse velocitites: ", t1.Elapsed<engine::milliseconds>().count());

        engine::Timer t2;
        t2.Start();
        SYCLProjectVelocities(px_b, py_b, x_b, y_b);
        engine::PrintLine("project velocitites1: ", t2.Elapsed<engine::milliseconds>().count());

        engine::Timer t3;
        t3.Start();
        SYCLAdvectVelocities();
        engine::PrintLine("advect velocitites: ", t3.Elapsed<engine::milliseconds>().count());

        engine::Timer t4;
        t4.Start();
        SYCLProjectVelocities(x_b, y_b, px_b, py_b);
        engine::PrintLine("project velocitites2: ", t4.Elapsed<engine::milliseconds>().count());

        engine::Timer t5;
        t5.Start();
        SYCLDiffuseDensities();
        engine::PrintLine("diffuse densities: ", t5.Elapsed<engine::milliseconds>().count());

        engine::Timer t6;
        t6.Start();
        SYCLAdvectDensities();
        engine::PrintLine("advect densities: ", t6.Elapsed<engine::milliseconds>().count());

    }


    void SYCLDiffuseVelocities() {
    ...
    void SYCLProjectVelocities(cl::sycl::buffer<float, 1>& px_b, cl::sycl::buffer<float, 1>& py_b, cl::sycl::buffer<float, 1>& x_b, cl::sycl::buffer<float, 1>& y_b) {
        Submit(queue, [&](cl::sycl::handler& cgh, auto x_a, auto px_a, auto y_a, auto py_a) {
